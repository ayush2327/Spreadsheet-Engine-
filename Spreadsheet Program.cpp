...
class segtree{
	private:
		struct Node{
			vector<int> seg;
			Node(const int& node , const int& null) : seg(2*node , null) {}
		};
	...
	private:
		void build(){
			...
			for(int i = 0; i < 2*node_out; i++){
				seg.emplace_back(node_in, null);
			}
		}
		...
		void update_row(const int& row , int i , const int& x){
			seg[row].seg[i] = x , i /= 2;
			while(i >= 1){
				merge(row , i);
				i>>=1;
		...
		int get_row(auto &row , int l , int r) const {
			int res = null;
			while(l <= r){
				if(l&1){
	...
	public:
	segtree(const int& n = 0 , const int& m = 0 , const int& null = 0 , const string& op = "NONE") : 
		n(n), m(m), null(null), operation(op) { build(); }

	public:
		void update(int i, int j, const int& x){
		...
		int get(int lx , int ly , int rx , int ry) const {
			int res = null;
...
class make_sheet {
	private: // instance of all variables and data stucture
		int R, C;
		int start_row = 1, start_col = 1;
		vector<vector<int>> sheet;
		vector<string> col;
		map<int , bool> vis;
		map<int , set<pair<int , string>>> graph , rev_graph; // node -> (node, formula) // keeping formula as edge weight
		segtree seg_sum , seg_min;
		segtree seg_max , seg_square_sum;

	private: // initialising Seg_Tree and converting string to column number
		void init(){
	...
	private: // cell and range parsing
	...
	public: // constructor
	    make_sheet(const int& r, const int& c) : R(r), C(c), sheet(r + 1, vector<int>(c + 1)), col(c + 1) {
	        col[0] = " ";
	        int l = 1;
	...
	public: // scrolling and assignment 
